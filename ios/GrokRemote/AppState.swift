...
/// Personal defaults for Feras's always-on bridge (Cloudflare → PC).
/// Token is NOT in the repo — enter once; Keychain keeps it.
enum FerasDefaults {
    static let bridgeURL = "https://tethrx.firashome.uk"
    static let computerName = "Feras-PC"
    static let appLabel = "TethrX · Feras"
}
...
final class AppState: ObservableObject {
    ...
    init() {
        ...
        // Prefer saved URL; first launch pre-fills Feras-PC Cloudflare bridge.
        let savedURL = d.string(forKey: "bridge.baseURL") ?? ""
        baseURLString = savedURL.isEmpty ? FerasDefaults.bridgeURL : savedURL
        ...
        // Seed a named computer entry so Settings shows "Feras-PC" even before first connect.
        if savedBridges.isEmpty {
            let seed = SavedBridge(
                id: "feras-pc",
                name: FerasDefaults.computerName,
                address: FerasDefaults.bridgeURL
            )
            savedBridges = [seed]
            activeBridgeId = seed.id
            persistBridges()
        }
    }
