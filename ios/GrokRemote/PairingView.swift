...
struct PairingView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            #endif
            // Feras build: skip install wizard — bridge already runs on the PC.
            // Jump to the scan/token step with address pre-filled.
            if app.baseURLString.isEmpty {
                app.baseURLString = FerasDefaults.bridgeURL
            }
            if let scanIdx = steps.firstIndex(of: .scan) {
                idx = scanIdx
            }
        }
    }
